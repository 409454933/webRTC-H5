...
let addUsedtx  = function(sdp, codec){
...
let modifyLevelId = function(parsedSdp, type,height){
    let levelIdc = null
    switch (Number(height)) {
        case 2160:
            levelIdc = '33'
            break
        case 1080:
...
let SDPTools = {
    getLocalSDP: function (SDP) {
        ...
        if(callType !== 'audioConference'){
            SDPTools.setMediaContentType(parsedSdp, ['main', 'slides'])
        }

        if(!RESOLUTION.VIDEO_CURRENT_UP){
            height = RESOLUTION.VIDEO_DEFAULT.height
        }else {
            height = RESOLUTION.VIDEO_CURRENT_UP.height
        }
        modifyLevelId(parsedSdp,'main',height)

        // delete codec
        This.trimCodec(parsedSdp)
        // todo: In order to send RTCP normally, all media lines need to carry ssrc
        // This.removeSSRC(parsedSdp)
        // save current ssrc and code for getStats
        This.renameMediaMsid(parsedSdp, gsRTC.accountId)
        ...
        let setMaxBitRate = gsRTC.getConfFromCookies('maxBitRate')
        if (setMaxBitRate !== 'auto') {
            log.info('remove local goog-remb and transport-cc')
        ...
        for (let i = 0; i < parsedSdp.media.length; i++) {
            ...
            if(i >= 3 && This.callType.indexOf('videoConference') >= 0){
                // In the video conference room, re-invite only brings its own three media lines
                parsedSdp.media.splice(i, 1)
                i--
            }else {
                ...
                if(This.action === 'hold'){
                    if(media.content === 'main'){
                        // todo: Only one video screen is displayed on the other side after hold
                        media.direction = This.isLocalVideoOn === true ? 'sendrecv' : 'inactive'
                    }else{
                        if(This.isRemoteHold){
                            if (parsedSdp.media[i].direction === 'sendrecv' || parsedSdp.media[i].direction === 'recvonly') {
                                parsedSdp.media[i].direction = 'inactive'
                            }
                        }else{
                            if (parsedSdp.media[i].direction === 'sendrecv') {
                                parsedSdp.media[i].direction = 'sendonly'
                            } else if (parsedSdp.media[i].direction === 'recvonly') {
                                parsedSdp.media[i].direction = 'inactive'
                            }
                        }
                    }
                }else if(This.action === 'unhold'){
                    if(media.type === 'audio' && This.isRemoteHold){
                    ...
                    if(media.type === 'audio'){
                        ...
                        media.direction = This.action === 'stopShareVideo' ? 'inactive' : 'recvonly'
                    }else if (media.content === 'slides'){
                    ...
                    if (media.content && media.content === 'main') {
                        ...
                        // The local video stream needs to be cleared when the requested end does not request to open the video for p2p
                        log.info('This.isRecvRequest: ' + This.isRecvRequest)
                        log.info('This.isRequestOpenLocalVideo: ' + This.isRequestOpenLocalVideo)
                        log.info('main stream: ', stream)

                        if(This.action === 'stopShareVideo'){
                            media.direction = 'inactive'
                        } else if ((This.isRecvRequest && !This.isRequestOpenLocalVideo) || !stream) {
                            media.direction = 'inactive'
                            if (stream) {
                            ...
                            if (This.callType.indexOf('Conference') >= 0) {
